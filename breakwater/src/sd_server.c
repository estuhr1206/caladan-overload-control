...
#define SSD_MAX_WINDOW		64

#define SSD_TRACK_FLOW		false
#define SSD_TRACK_FLOW_ID	1

#define SSD_TS_OUT		false
...
BUILD_ASSERT((1 << SSD_MAX_WINDOW_EXP) == SSD_MAX_WINDOW);
...
struct ssd_session {
	...
	DEFINE_BITMAP(avail_slots, SSD_MAX_WINDOW);

	/* shared state between workers and sender */
	spinlock_t		lock;
	int			closed;
	thread_t		*sender_th;
	DEFINE_BITMAP(completed_slots, SSD_MAX_WINDOW);

	/* worker slots (one for each credit issued) */
	struct ssd_ctx		*slots[SSD_MAX_WINDOW];
};
...
static int srpc_get_slot(struct ssd_session *s)
{
	int slot = __builtin_ffsl(s->avail_slots[0]) - 1;
	if (slot >= 0) {
		bitmap_atomic_clear(s->avail_slots, slot);
		s->slots[slot] = smalloc(sizeof(struct ssd_ctx));
...
static void srpc_put_slot(struct ssd_session *s, int slot)
{
	sfree(s->slots[slot]);
	s->slots[slot] = NULL;
...
static int srpc_send_completion_vector(struct ssd_session *s,
				       unsigned long *slots)
{
	struct ssd_hdr shdr[SSD_MAX_WINDOW];
	struct iovec v[SSD_MAX_WINDOW * 2];
	int nriov = 0;
	int nrhdr = 0;
	int i;
	ssize_t ret = 0;

	bitmap_for_each_set(slots, SSD_MAX_WINDOW, i) {
		struct ssd_ctx *c = s->slots[i];

		shdr[nrhdr].magic = SD_RESP_MAGIC;
		shdr[nrhdr].op = SD_OP_CALL;
		shdr[nrhdr].len = c->cmn.resp_len;
		shdr[nrhdr].id = c->cmn.id;
		shdr[nrhdr].ts = c->ts;

		v[nriov].iov_base = &shdr[nrhdr];
		v[nriov].iov_len = sizeof(struct ssd_hdr);
		nrhdr++;
		nriov++;
...
{
	struct ssd_ctx *c = (struct ssd_ctx *)arg;
	struct ssd_session *s = (struct ssd_session *)c->cmn.s;
...
static int srpc_recv_one(struct ssd_session *s)
{
	struct csd_hdr chdr;
	int idx, ret;
...
again:
	...
	if (unlikely(chdr.magic != SD_REQ_MAGIC)) {
...
{
	DEFINE_BITMAP(tmp, SSD_MAX_WINDOW);
	struct ssd_session *s = (struct ssd_session *)arg;
	int i;
	bool sleep;
	int num_resp;
	...
	while (true) {
		...
		bitmap_init(s->completed_slots, SSD_MAX_WINDOW, false);

		num_resp = bitmap_popcount(tmp, SSD_MAX_WINDOW);
	...
	spin_lock_np(&s->lock);
	while (!s->closed ||
	       bitmap_popcount(s->avail_slots, SSD_MAX_WINDOW) +
	       bitmap_popcount(s->completed_slots, SSD_MAX_WINDOW) <
	       SSD_MAX_WINDOW) {
		s->sender_th = thread_self();
		thread_park_and_unlock_np(&s->lock);
		spin_lock_np(&s->lock);
 ...
 */
int ssd_enable(srpc_fn_t handler)
{
...
uint64_t ssd_stat_req_dropped()
{
	return atomic64_read(&srpc_stat_req_dropped_);
}

uint64_t ssd_stat_resp_tx()
{
